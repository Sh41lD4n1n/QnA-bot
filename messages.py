...
def get_text_from_file(key=[0,0]):
    f=open('textOfMessages.txt','r')
    f.readlines()
    f.close()
...
async def function_HELP (language:str="b",state:str=None)->Attachment:
        ...
        if language=="English":
            ...
            description_for_command=[
                "This command start conversation with bot",      #/start
                "This command open menu of setting (There you can change language). This command available after initialisation and after you complete to write question", #setting
                "This command open 10 tickets opened and closed",     #ticket
                "This command allow you to skip some steps of question creation and feedback writing",#skip
                "Type this command allow you to close question" #cencel
            ]
        elif language=="Русский":
            text = "Помощь"
            ...
            description_for_command = [
                ...
                # setting
                "Эта команда отображает полный список из 10 запросов отправленных оператору(открытых и завершенных)",  # ticket
                "Пропустить некоторые шаги при создании вопроса или составлении отзыва, можно с помщью этой команды",  # skip
